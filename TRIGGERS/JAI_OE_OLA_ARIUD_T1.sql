--------------------------------------------------------
--  DDL for Trigger JAI_OE_OLA_ARIUD_T1
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "APPS"."JAI_OE_OLA_ARIUD_T1" 
AFTER INSERT OR UPDATE OR DELETE ON "ONT"."OE_ORDER_LINES_ALL"
FOR EACH ROW
DECLARE
...
BEGIN
  ...
  */
  ...
  IF inserting THEN
    ...
    IF (
         (
             (  nvl(:new.ship_to_org_id,0)      <> nvl(:old.ship_to_org_id,0)         OR
                 :old.ship_to_org_id            IS NULL
             )                                                                        AND
             nvl(:new.flow_status_code,'###')   NOT IN ('CANCELLED','CLOSED')
         )                                                                            OR
         (
             nvl(:old.ordered_quantity, 0)      = 0                                   AND
             nvl(:new.ordered_quantity, 0)      <> 0
         )                                                                            OR
         (   :old.inventory_item_id             <> :new.inventory_item_id          )  OR
         (   nvl(:old.line_number,-9999)        <> nvl(:new.line_number,-9998)     )  OR
         (   nvl(:old.shipment_number,-9999)    <> nvl(:new.shipment_number,-9998) )
       )
    THEN

      jai_oe_ola_trigger_pkg.ariu_t1 (
                                        pr_old            =>  t_old_rec         ,
                                        pr_new            =>  t_new_rec         ,
                                        pv_action         =>  lv_action         ,
...
/*Commented by aiyer for the bug 5401180
    IF  ( (   nvl(:new.ordered_quantity,0)   <> nvl(:old.ordered_quantity,0)    )    OR
          (   nvl(:new.cancelled_quantity,0) <> nvl(:old.cancelled_quantity,0)  )    OR
          (   nvl(:new.unit_selling_price,0) <> nvl(:old.unit_selling_price,0)  )    OR
          (
            ( :new.inventory_item_id         <> :old.inventory_item_id)              AND
            ( :new.line_category_code        = 'RETURN' )                            AND
            --( :new.return_context            = 'LEGACY')
          )
        )
    THEN
*/
    IF  ( (   nvl(:new.ordered_quantity,0)   <> nvl(:old.ordered_quantity,0)    )    OR
          ...
          (
            ( :new.inventory_item_id         <> :old.inventory_item_id)              AND
            ( :new.line_category_code        = 'RETURN' )                            AND
            ( :new.return_context            IS NULL    ) /*changed by aiyer for the bug 5401180 from legacy to null  */
          )
        )
    THEN

      jai_oe_ola_trigger_pkg.aru_t1 (
                                      pr_old            =>  t_old_rec         ,
                                      pr_new            =>  t_new_rec         ,
                                      pv_action         =>  lv_action         ,
                                      pv_return_code    =>  lv_return_code    ,
    ...
    /* for fulfilled non-shippable item, need to populate JAI tables for VAT invoice
       number generation and Fulfilled Order form.
    */
    IF (
         (nvl(:old.fulfilled_flag, 'N') <> 'Y' AND nvl(:new.fulfilled_flag, 'N') = 'Y') AND
         nvl(:new.line_category_code, 'RETURN') = 'ORDER'                               AND
         nvl(:new.shipping_interfaced_flag, 'Y') = 'N'
       )
    THEN
      jai_oe_ola_trigger_pkg.ariu_t2( pr_old            =>  t_old_rec
                                    , pr_new            =>  t_new_rec
                                    , pv_action         =>  lv_action
                                    , pv_return_code    =>  lv_return_code
                                    , pv_return_message =>  lv_return_message
    ...
    END IF; -- (nvl(:old.fulfilled_flag, 'N') <> 'Y' ......
...
EXCEPTION
  ...
  WHEN OTHERS THEN
      ...
      app_exception.raise_exception (
                                      EXCEPTION_TYPE  => 'APP',
                                      EXCEPTION_CODE  => -20110 ,
                                      EXCEPTION_TEXT  => 'Encountered the error in trigger JAI_OE_OLA_ARIUD_T1' || substr(sqlerrm,1,1900)
                                    );

END JAI_OE_OLA_ARIUD_T1 ;

/
ALTER TRIGGER "APPS"."JAI_OE_OLA_ARIUD_T1" DISABLE;
