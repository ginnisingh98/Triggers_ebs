--------------------------------------------------------
--  DDL for Trigger PERPERSONTYPEUSAGESF_153I_DYT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "APPS"."PERPERSONTYPEUSAGESF_153I_DYT" AFTER INSERT ON "HR"."PER_PERSON_TYPE_USAGES_F" FOR EACH ROW DECLARE 
...
/*
  ...
  ------------------------------------------------
    ...
    Info.:  Incident Register trigger on insert of person types usage
  ================================================
*/
--
...
END IF;
  ...
  pay_misc_dyt_incident_pkg.per_person_type_usages_f_ari(
    p_business_group_id            => l_business_group_id,
    p_effective_start_date         => :new.effective_start_date,
    p_legislation_code             => l_legislation_code,
    p_person_id                    => :new.person_id,
    p_person_type_usage_id         => :new.person_type_usage_id
  );
  --
...
EXCEPTION
  WHEN OTHERS THEN
    hr_utility.set_location('PERPERSONTYPEUSAGESF_153I_DYT',ABS(SQLCODE));
    pay_dyn_triggers.g_dyt_mode := l_mode;
    RAISE;
  --
END;
...
/
ALTER TRIGGER "APPS"."PERPERSONTYPEUSAGESF_153I_DYT" ENABLE;
