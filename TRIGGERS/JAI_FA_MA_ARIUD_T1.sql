--------------------------------------------------------
--  DDL for Trigger JAI_FA_MA_ARIUD_T1
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "APPS"."JAI_FA_MA_ARIUD_T1" 
AFTER INSERT OR UPDATE OR DELETE ON "FA"."FA_MASS_ADDITIONS"
FOR EACH ROW
DECLARE
...
BEGIN
  ...
  */
  ...
  IF INSERTING THEN
    ...
    IF ( upper(:NEW.feeder_system_name) = 'ORACLE PAYABLES' ) THEN

      JAI_FA_MA_TRIGGER_PKG.ARI_T1 (
                        pr_old            =>  t_old_rec         ,
                        pr_new            =>  t_new_rec         ,
                        pv_action         =>  lv_action         ,
...
EXCEPTION
  ...
  WHEN OTHERS THEN
      ...
      app_exception.raise_exception (
                                      EXCEPTION_TYPE  => 'APP',
                                      EXCEPTION_CODE  => -20110 ,
                                      EXCEPTION_TEXT  => 'Encountered the error in trigger JAI_FA_MA_ARIUD_T1' || substr(sqlerrm,1,1900)
                                    );

END JAI_FA_MA_ARIUD_T1 ;


/
ALTER TRIGGER "APPS"."JAI_FA_MA_ARIUD_T1" DISABLE;
