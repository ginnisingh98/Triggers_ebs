--------------------------------------------------------
--  DDL for Trigger JAI_AP_IDA_ARIUD_T1
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "APPS"."JAI_AP_IDA_ARIUD_T1" 
AFTER INSERT OR UPDATE OR DELETE ON "AP"."AP_INVOICE_DISTRIBUTIONS_ALL"
FOR EACH ROW
DECLARE
...
BEGIN
/*------------------------------------------------------------------------------------------------------------------
FILENAME: jai_ap_ida_t.sql
CHANGE HISTORY:

S.No      Date          Author and Details
----------------------------------------------
1         26-Dec-2007   Jia Li
                        for Tax Inclusive Computations.

2        23-jan-2008   File being merged from version 120.5. Changes done by nprashar.
-------------------------------------------------------------------------------------------------------------------*/
  ...
  */
  IF jai_cmn_utils_pkg.check_jai_exists(P_CALLING_OBJECT => 'JAI_AP_IDA_ARIUD_T1', P_ORG_ID => :new.org_id, p_set_of_books_id => :new.set_of_books_id ) = FALSE THEN
       RETURN;
  END IF;
  ...
  */
  ...
  IF INSERTING THEN
      ...
      JAI_AP_IDA_TRIGGER_PKG.ARUID_T1 (
                        pr_old            =>  t_old_rec         ,
                        pr_new            =>  t_new_rec         ,
                        pv_action         =>  lv_action         ,
  ...
  IF UPDATING THEN
      ...
      -- Added by Jia Li for inclusive tax on 2007/12/26, Begin
      ----------------------------------------------------------
      IF t_new_rec.posted_flag = 'Y' AND t_old_rec.posted_flag = 'N'
      THEN
        jai_ap_utils_pkg.acct_inclu_taxes( pn_invoice_id              => t_new_rec.invoice_id
                                         , pn_invoice_distribution_id => t_new_rec.invoice_distribution_id
      ...
      END IF;
      ----------------------------------------------------------
      -- Added by Jia Li for inclusive tax on 2007/12/26, End
...
EXCEPTION
  ...
  WHEN OTHERS THEN
      ...
      app_exception.raise_exception (
                                      EXCEPTION_TYPE  => 'APP',
                                      EXCEPTION_CODE  => -20110 ,
                                      EXCEPTION_TEXT  => 'Encountered the error in trigger JAI_AP_IDA_ARIUD_T1' || substr(sqlerrm,1,1900)
                                    );

END JAI_AP_IDA_ARIUD_T1 ;

/
ALTER TRIGGER "APPS"."JAI_AP_IDA_ARIUD_T1" DISABLE;
