--------------------------------------------------------
--  DDL for Trigger PERADDRESSES_137I_DYT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "APPS"."PERADDRESSES_137I_DYT" AFTER INSERT ON "HR"."PER_ADDRESSES" FOR EACH ROW DECLARE 
...
/*
  ...
  ------------------------------------------------
    ...
    Info.:  Continuous Calculation trigger on insert of Address.
  ================================================
*/
--
...
END IF;
  ...
  pay_continuous_calc.per_addresses_ari(
    p_business_group_id            => l_business_group_id,
...
EXCEPTION
  WHEN OTHERS THEN
    hr_utility.set_location('PERADDRESSES_137I_DYT',ABS(SQLCODE));
    pay_dyn_triggers.g_dyt_mode := l_mode;
    RAISE;
  --
END;
...
/
ALTER TRIGGER "APPS"."PERADDRESSES_137I_DYT" ENABLE;
