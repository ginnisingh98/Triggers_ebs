--------------------------------------------------------
--  DDL for Trigger PERALLPEOPLEF_57U_DYT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "APPS"."PERALLPEOPLEF_57U_DYT" AFTER UPDATE ON "HR"."PER_ALL_PEOPLE_F" FOR EACH ROW DECLARE 
...
/*
  ...
  ------------------------------------------------
    ...
    Info.:  Continuous Calculation trigger on update of Person
  ================================================
*/
--
...
EXCEPTION
  WHEN OTHERS THEN
    hr_utility.set_location('PERALLPEOPLEF_57U_DYT',ABS(SQLCODE));
    pay_dyn_triggers.g_dyt_mode := l_mode;
    RAISE;
  --
END;
...
/
ALTER TRIGGER "APPS"."PERALLPEOPLEF_57U_DYT" ENABLE;
