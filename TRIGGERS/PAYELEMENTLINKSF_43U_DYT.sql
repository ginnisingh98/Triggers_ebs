--------------------------------------------------------
--  DDL for Trigger PAYELEMENTLINKSF_43U_DYT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "APPS"."PAYELEMENTLINKSF_43U_DYT" AFTER UPDATE ON "HR"."PAY_ELEMENT_LINKS_F" FOR EACH ROW DECLARE 
...
/*
  ...
  ------------------------------------------------
    ...
    Info.:  Continuous Calculation trigger on update of element link
  ================================================
*/
--
...
EXCEPTION
  WHEN OTHERS THEN
    hr_utility.set_location('PAYELEMENTLINKSF_43U_DYT',ABS(SQLCODE));
    pay_dyn_triggers.g_dyt_mode := l_mode;
    RAISE;
  --
END;
...
/
ALTER TRIGGER "APPS"."PAYELEMENTLINKSF_43U_DYT" ENABLE;
