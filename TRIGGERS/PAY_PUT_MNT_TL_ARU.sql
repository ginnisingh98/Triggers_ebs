--------------------------------------------------------
--  DDL for Trigger PAY_PUT_MNT_TL_ARU
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "APPS"."PAY_PUT_MNT_TL_ARU" 
...
if ( hr_general.g_data_migrator_mode <> 'Y' and
 ...
 pay_ptt_upd.upd_tl(     P_LANGUAGE_CODE        =>userenv('LANG')
                        ,P_USER_TABLE_ID        =>:NEW.user_table_id
                        ,P_USER_TABLE_NAME      =>:NEW.user_table_name
                        ,P_USER_ROW_TITLE       =>:NEW.user_row_title );

end if;
end pay_put_mnt_tl_aru;


/
ALTER TRIGGER "APPS"."PAY_PUT_MNT_TL_ARU" ENABLE;
