--------------------------------------------------------
--  DDL for Trigger PAY_ELEMENT_SETS_MNT_TL_ARU
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "APPS"."PAY_ELEMENT_SETS_MNT_TL_ARU" 
...
if ( hr_general.g_data_migrator_mode <> 'Y') and
 ...
 pay_est_upd.upd_tl(P_LANGUAGE_CODE =>userenv('LANG'),
                              P_ELEMENT_SET_ID => :NEW.element_set_id,
			      P_ELEMENT_SET_NAME => :NEW.element_set_name );

end if;
end if;
end pay_element_sets_mnt_tl_aru;

/
ALTER TRIGGER "APPS"."PAY_ELEMENT_SETS_MNT_TL_ARU" ENABLE;
