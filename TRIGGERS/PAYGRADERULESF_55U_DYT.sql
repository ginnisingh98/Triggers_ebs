--------------------------------------------------------
--  DDL for Trigger PAYGRADERULESF_55U_DYT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "APPS"."PAYGRADERULESF_55U_DYT" AFTER UPDATE ON "HR"."PAY_GRADE_RULES_F" FOR EACH ROW DECLARE 
...
/*
  ...
  ------------------------------------------------
    ...
    Info.:  Continuous Calculation trigger on update of Grade Rules
  ================================================
*/
--
...
EXCEPTION
  WHEN OTHERS THEN
    hr_utility.set_location('PAYGRADERULESF_55U_DYT',ABS(SQLCODE));
    pay_dyn_triggers.g_dyt_mode := l_mode;
    RAISE;
  --
END;
...
/
ALTER TRIGGER "APPS"."PAYGRADERULESF_55U_DYT" ENABLE;
