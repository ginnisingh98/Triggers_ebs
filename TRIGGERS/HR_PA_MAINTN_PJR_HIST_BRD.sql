--------------------------------------------------------
--  DDL for Trigger HR_PA_MAINTN_PJR_HIST_BRD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "APPS"."HR_PA_MAINTN_PJR_HIST_BRD" 
BEFORE DELETE
...
BEGIN
  ...
  IF (PA_INSTALL.is_prm_licensed = 'Y' OR PA_INSTALL.is_utilization_implemented = 'Y') THEN
          ...
          pa_hr_update_pa_entities.update_project_entities(
            ...
            ,p_to_job_id_new             => NULL
            ,p_to_job_id_old             => :old.to_job_id
            ,p_from_job_id_new           => NULL
            ,p_from_job_id_old           => :old.from_job_id
            ,p_to_job_group_id           => :old.to_job_group_id
            ,p_from_job_group_id         => :old.from_job_group_id
...
/
ALTER TRIGGER "APPS"."HR_PA_MAINTN_PJR_HIST_BRD" ENABLE;
