--------------------------------------------------------
--  DDL for Trigger JAI_JPO_LLT_BRIUD_T1
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "APPS"."JAI_JPO_LLT_BRIUD_T1" 
BEFORE INSERT OR UPDATE OR DELETE ON "JA"."JAI_PO_TAXES"
FOR EACH ROW
DECLARE
  t_old_rec             JAI_PO_TAXES%rowtype ;
  t_new_rec             JAI_PO_TAXES%rowtype ;
  ...
  */
  ...
  BEGIN
    ...
    t_new_rec.LINE_FOCUS_ID                            := :new.LINE_FOCUS_ID                                 ;
  ...
  BEGIN
    ...
    t_old_rec.LINE_FOCUS_ID                            := :old.LINE_FOCUS_ID                                 ;
  ...
  */
  ...
  BEGIN
    ...
    :new.LINE_FOCUS_ID                            := t_new_rec.LINE_FOCUS_ID                                 ;
  ...
  */
  ...
  IF UPDATING THEN
    ...
    IF ( :OLD.tax_category_id IS NOT NULL AND :OLD.tax_id <> :NEW.tax_id ) THEN
      ...
      JAI_JPO_LLT_TRIGGER_PKG.BRU_T1 (
                        pr_old            =>  t_old_rec         ,
                        pr_new            =>  t_new_rec         ,
                        pv_action         =>  lv_action         ,
...
EXCEPTION
  ...
  WHEN OTHERS THEN
      ...
      app_exception.raise_exception (
                                      EXCEPTION_TYPE  => 'APP',
                                      EXCEPTION_CODE  => -20110 ,
                                      EXCEPTION_TEXT  => 'Encountered the error in trigger JAI_JPO_LLT_BRIUD_T1' || substr(sqlerrm,1,1900)
                                    );

END JAI_JPO_LLT_BRIUD_T1 ;


/
ALTER TRIGGER "APPS"."JAI_JPO_LLT_BRIUD_T1" DISABLE;
