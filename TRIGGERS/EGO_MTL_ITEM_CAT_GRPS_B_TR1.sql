--------------------------------------------------------
--  DDL for Trigger EGO_MTL_ITEM_CAT_GRPS_B_TR1
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "APPS"."EGO_MTL_ITEM_CAT_GRPS_B_TR1" 
...
ON "INV"."MTL_ITEM_CATALOG_GROUPS_B"
FOR EACH ROW

DECLARE
   l_table_name          VARCHAR2(30)    :=  'MTL_ITEM_CATALOG_GROUPS_B';
   l_event               VARCHAR2(30);
   l_scope               VARCHAR2(30)    :=  'ROW';
   l_process_event       BOOLEAN         :=  FALSE;

   TYPE Segment_Tbl_Type IS TABLE OF MTL_ITEM_CATALOG_GROUPS_B.SEGMENT1%TYPE
                            INDEX BY BINARY_INTEGER;

   Old_Segment_Tbl       Segment_Tbl_Type;
   New_Segment_Tbl       Segment_Tbl_Type;

   Old_Category_Name     VARCHAR2(2000)  :=  NULL;
   New_Category_Name     VARCHAR2(2000)  :=  NULL;

   l_application_id      NUMBER(10)      :=  401;
   l_id_flex_code        VARCHAR2(4)     :=  'MICG';
...
BEGIN
   ...
   IF ( UPDATING ) THEN
      ...
      LOOP
         ...
         IF ( New_Category_Name IS NOT NULL ) THEN
            Old_Category_Name := Old_Category_Name || l_delimiter || Old_Segment_Tbl(l_seg_col_num);
            New_Category_Name := New_Category_Name || l_delimiter || New_Segment_Tbl(l_seg_col_num);
         ELSE
            Old_Category_Name := Old_Segment_Tbl(l_seg_col_num);
            New_Category_Name := New_Segment_Tbl(l_seg_col_num);
         END IF;
      END LOOP;

      IF ( New_Category_Name <> Old_Category_Name ) THEN
   ...
   IF ( l_process_event ) THEN
      ...
      ,  p_item_catalog_group_id   =>  :old.ITEM_CATALOG_GROUP_ID
      );
...
EXCEPTION
   WHEN others THEN
      EGO_ITEM_TEXT_PVT.Log_Line('EGO_MTL_ITEM_CAT_GRPS_B_TR1: Error: '||SQLERRM);

END EGO_MTL_ITEM_CAT_GRPS_B_TR1;


/
ALTER TRIGGER "APPS"."EGO_MTL_ITEM_CAT_GRPS_B_TR1" ENABLE;
