--------------------------------------------------------
--  DDL for Trigger HR_PAY_IF_ASG_COST_BRD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "APPS"."HR_PAY_IF_ASG_COST_BRD" BEFORE DELETE
ON "HR"."PAY_COST_ALLOCATIONS_F" FOR EACH ROW
DECLARE
  --
  ...
  CURSOR csr_get_event_id IS
    ...
    WHERE short_name = 'HR_PAY_IF_ASG_COST_BRD';
  --
...
HR_PAY_INTERFACE_PKG.g_reporting_details_rec_var.business_group_id;
    ...
    IF hr_pay_interface_pkg.g_cost_allocation_id IS NOT NULL AND
      :old.cost_allocation_id <>
      hr_pay_interface_pkg.g_cost_allocation_id
    THEN
      hr_utility.set_message (800, 'PER_ASG_COST_INF_DIS_DEL');
      hr_pay_interface_pkg.g_cost_allocation_id := NULL;
      hr_utility.raise_error;
    ELSE
      hr_pay_interface_pkg.g_cost_allocation_id := :old.cost_allocation_id;
      hr_pay_interface_pkg.g_asg_cost_ass_id    := :old.assignment_id;
--      hr_pay_interface_pkg.g_asg_cost_end_date  := :old.effective_end_date;
      hr_pay_interface_pkg.g_asg_cost_start_date  := :old.effective_start_date;
    END IF;
  END IF;
END hr_pay_if_asg_cost_brd;


/
ALTER TRIGGER "APPS"."HR_PAY_IF_ASG_COST_BRD" ENABLE;
