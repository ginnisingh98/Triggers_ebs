--------------------------------------------------------
--  DDL for Trigger XTR_AI_MIRROR_DDA_LIMIT_ROW_T
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "APPS"."XTR_AI_MIRROR_DDA_LIMIT_ROW_T" 
 AFTER INSERT on "XTR"."XTR_MIRROR_DDA_LIMIT_ROW" FOR EACH ROW
--
declare
  ...
  select 1
    from XTR_GROUP_LIMITS
     where COMPANY_CODE = :NEW.COMPANY_CODE
      ...
      for update of UTILISED_AMOUNT NOWAIT;
begin
xtr_debug_pkg.debug('Before XTR_AI_XTR_MIRROR_DDA_LIMIT_ROW_T on:'||
...
xtr_debug_pkg.debug('After XTR_AI_XTR_MIRROR_DDA_LIMIT_ROW_T on:'||
 to_char(sysdate,'MM:DD:HH24:MI:SS'));
exception
...
/
ALTER TRIGGER "APPS"."XTR_AI_MIRROR_DDA_LIMIT_ROW_T" ENABLE;
