--------------------------------------------------------
--  DDL for Trigger BOM_STD_SUB_OP_RESOURCES_T2
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "APPS"."BOM_STD_SUB_OP_RESOURCES_T2" 
	BEFORE UPDATE ON "BOM"."BOM_STD_SUB_OP_RESOURCES" FOR EACH ROW
...
BEGIN
	...
	SET   substitute_group_num = :new.substitute_group_num,
              schedule_seq_num =  :new.schedule_seq_num, -- Bug 7370692  null --after making this column nullable in the table
              replacement_group_num = :new.replacement_group_num,
	...
	AND   substitute_group_num = :old.substitute_group_num
        AND   replacement_group_num = :old.replacement_group_num;--bug3433654
    END LOOP;

END;

/
ALTER TRIGGER "APPS"."BOM_STD_SUB_OP_RESOURCES_T2" ENABLE;
