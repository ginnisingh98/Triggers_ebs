--------------------------------------------------------
--  DDL for Trigger JAI_AP_IA_BRIUD_T1
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "APPS"."JAI_AP_IA_BRIUD_T1" 
BEFORE INSERT OR UPDATE OR DELETE ON "AP"."AP_INVOICES_ALL"
FOR EACH ROW
DECLARE
...
BEGIN
  ...
  */
  IF jai_cmn_utils_pkg.check_jai_exists(P_CALLING_OBJECT => 'JAI_AP_IA_BRIUD_T1', P_ORG_ID => :new.org_id, p_set_of_books_id => :new.set_of_books_id ) = FALSE THEN
       RETURN;
  END IF;
  ...
  */
  ...
  IF INSERTING THEN
      ...
      JAI_AP_IA_TRIGGER_PKG.BRIUD_T1 (
                        pr_old            =>  t_old_rec         ,
                        pr_new            =>  t_new_rec         ,
                        pv_action         =>  lv_action         ,
...
EXCEPTION
  ...
  WHEN OTHERS THEN
      ...
      app_exception.raise_exception (
                                      EXCEPTION_TYPE  => 'APP',
                                      EXCEPTION_CODE  => -20110 ,
                                      EXCEPTION_TEXT  => 'Encountered the error in trigger JAI_AP_IA_BRIUD_T1' || substr(sqlerrm,1,1900)
                                    );

END JAI_AP_IA_BRIUD_T1 ;

/
ALTER TRIGGER "APPS"."JAI_AP_IA_BRIUD_T1" DISABLE;
